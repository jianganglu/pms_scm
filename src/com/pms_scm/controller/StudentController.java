...
//	}
	...
	@RequestMapping(params = "dataGrid")
	public void datagrid(Student student,HttpServletRequest reqeust, HttpServletResponse response, DataGrid dataGrid){
		CriteriaQuery cq = new CriteriaQuery(Student.class, dataGrid);
	...
	@ResponseBody
	public AjaxJson del(Student user, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
	...
	@RequestMapping(params = "del")
	@ResponseBody
	public AjaxJson del(Student student, HttpServletRequest req){
		AjaxJson j = new AjaxJson();
		if (student.getId()!=null){
			student = systemService.getEntity(Student.class, student.getId());
			message = "Demo: " + student.getSname() + "被删除 成功";
			// 删除部门之前更新与之相关的实体
			//upEntity(demo);
			systemService.delete(student);
			systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
		}
		j.setMsg(message);
		return j;
	...
	@ResponseBody
	public AjaxJson saveStudent(HttpServletRequest req, Student student){
		AjaxJson j = new AjaxJson();
		systemService.save(student);
		j.setMsg("添加成功");
		return j;
	}
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		return new ModelAndView("pms_scm/student");
